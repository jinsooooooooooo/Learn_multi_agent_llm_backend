...
# ===================================================================
# Chats 모델: llm_agent.chats 테이블과 매핑됩니다.
# ===================================================================
class Chats(OrmBase):
    ...
    # chat_id: uuid 타입의 기본 키(Primary Key)입니다.
    # DB에 이미 DEFAULT gen_random_uuid()가 설정되어 있으므로, 여기선 default 옵션을 주지 않습니다.
    # SQLAlchemy는 INSERT 시 이 컬럼을 생략하고, DB가 값을 채우도록 합니다.
    chat_id = Column(UUID(as_uuid=True), primary_key=True, server_default="gen_random_uuid")
    ...
    # 'Chats' 객체 하나는 여러 개의 'Messages' 객체를 가질 수 있습니다.
    messages = relationship(
        "Messages",               # 연결될 상대방 클래스의 이름입니다.
        back_populates="chat",       # 상대방(Messages)의 'chat' 속성과 서로 연결되어 있음을 명시합니다.
        primaryjoin="Chats.chat_id == Messages.chat_id",   #<클래스>.<속성> 으로 표현하여 join key 매핑
        cascade="all, delete-orphan"    # Chats이 삭제될 때, 관련된 모든 Messages도 함께 삭제되도록 하는 중요한 옵션입니다.
    )


# ===================================================================
# Messages 모델: llm_agent.messages 테이블과 매핑됩니다.
# ===================================================================
class Messages(OrmBase):
    ...
    # chat_id: 이 메시지가 어떤 Chats에 속하는지를 가리키는 외래 키(Foreign Key)입니다.
    chat_id =  Column(UUID(as_uuid=True), ForeignKey('llm_agent.chats.chat_id'), nullable=False)
    ...
    created_at = Column(DateTime(timezone=True), server_default="now()")
     ...
     # --- 관계(Relationship) 정의 ---
    # 'Messages' 객체는 하나의 'Chats' 객체에 속합니다.
    chat = relationship(
        "Chats",                  # 연결될 상대방 클래스의 이름입니다.
        back_populates="messages"       # 상대방(Chats)의 'messages' 속성과 서로 연결되어 있음을 명시합니다.
    )
